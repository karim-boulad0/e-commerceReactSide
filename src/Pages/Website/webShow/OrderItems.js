...
const OrderItems = () => {
  ...
  useEffect(() => {
    Axios.get("/webSite/orderItems/index")
      ...
      .catch((err) => {
        console.log(err); setIsGetOrderItems(true);
      });
  }, [action]);
  ...
  return (
    <Container>
      ...
      {isGetOrderItems && orderItems.length > 0 ? (<><div className="btn btn-primary" onClick={() => setShowDeleteConfirmation(true)}>
        confirm all order items
      </div>  <Row xs={1} sm={2} md={3} lg={3} className="g-4 mt-4">
          {showOrdersItems}
        </Row></>) : <h1 className="alert alert-primary">no order Items found</h1>}
      {/* delete modal */}
      <Modal show={showDeleteConfirmation} onHide={handleDeleteClose}>
        <Modal.Header closeButton>
          <Modal.Title>Confirmed Order Items </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>Are you sure you want to confirmed all order items</p>
        </Modal.Body>
